...
# Two CTA patterns exist across the blog:
#   OLD posts use <div class="cta-block"> with a .cta-btn anchor inside.
#   NEW posts use <div class="post-cta"> with a .btn-primary anchor inside,
#     and the link points DIRECTLY to the GHL booking URL (bypassing /contact).
# Both get replaced with the same newsletter-first CTA.
# Non-greedy + DOTALL handles single-line AND multi-line variants. Neither
# CTA contains nested <div> tags, so .*?</div> is safe.
PATTERNS = [
    re.compile(
    ...
    re.compile(
        r'<div class="post-cta">.*?class="btn-primary">.*?</a>\s*</div>',
        re.DOTALL,
    ),
]

# Marker for idempotency. If the new CTA is already in the file, skip it.
...
def main() -> int:
    ...
    for path in sorted(blog_dir.glob('*.html')):
        ...
        if ALREADY_DONE_MARKER in text:
            ...
            continue
        if 'class="cta-block"' not in text and 'class="post-cta"' not in text:
        ...
        for pattern in PATTERNS:
            new_text, count = pattern.subn(NEW_CTA, new_text)
            total_count += count
        ...
        changed.append((path.name, total_count))
